...
// Article 생성: POST
userRoutes.post("/", verificationMiddleware)
...
userRoutes.put("/:article_id", verificationMiddleware)
userRoutes.put("/:article_id", async (req: express.Request, res: express.Response) => {
...
userRoutes.delete("/:article_id", verificationMiddleware)
userRoutes.delete("/:article_id", async (req: express.Request, res: express.Response) => {
  const article_id = req.params["article_id"]
  const requestBody = req.body
  ArticleModel.findOne({ _id: article_id }, async (err: Error, article: any) => {
    if (err) {
...
userRoutes.get("/:article_id/candidates", async (req: express.Request, res: express.Response) => {
  const article_id = req.params["article_id"]
  ArticleModel.findOne({ _id: article_id }, async (err: Error, article: any) => {
    if (err) {
      console.log(err)
...
userRoutes.post("/:article_id/candidates", verificationMiddleware)
userRoutes.post("/:article_id/candidates", async (req: express.Request, res: express.Response) => {
  const article_id = req.params["article_id"]
  ArticleModel.findOne({ _id: article_id }, async (err: Error, article: any) => {
    if (err) {
      console.log(err)
